...
function loadRooms(callback) {
	db.query("SELECT id, name FROM rooms", function (err, result) {
		...
		result.rows.forEach(function (info) {
			rooms[info.id] = new Room(db, info);
			console.log("Created room '" + info.name + "'");
		});
			...
			if (row.type in classes) {
				...
				db.query(
					...
					function (err2, result2) {
						...
						var instance = new classes[row.type](row.conf, slots);
						var entity = new Entity(db, row, instance, comm);
